...
// TODO(Trevor): These asserts are wrong. Could vary depending on transpose args.
template <>
void quark_gpu_geam(cublasHandle_t handle, const float* alpha, bool trans_a, const GpuTensor<float>&a,
  ...
  int lda = trans_a ? a.shape()[1] : a.shape()[0];
  int ldb = trans_b ? b.shape()[1] : b.shape()[0];
  ...
  CUBLAS_CALL(cublasSgeam(handle, transpose_a , transpose_b, a.shape()[0], a.shape()[1], alpha, a.data(), lda,
          beta, b.data(), ldb, c->mutable_data(), c->shape()[0]));
}
...
void quark_gpu_geam(cublasHandle_t handle, const double* alpha, bool trans_a, const GpuTensor<double>&a,
  ...
  cublasOperation_t transpose_a = trans_a ? CUBLAS_OP_T : CUBLAS_OP_N;
  cublasOperation_t transpose_b = trans_b ? CUBLAS_OP_T : CUBLAS_OP_N;
  CUBLAS_CALL(cublasDgeam(handle, transpose_a , transpose_b, a.shape()[0], a.shape()[1], alpha, a.data(), lda,
          beta, b.data(), ldb, c->mutable_data(), c->shape()[0]));
}
